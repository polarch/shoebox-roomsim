...
mic_specs = [ 1 0 0 0.5;    % cardioid looking to the front
...
% compute echograms
[abs_echograms, rec_echograms, echograms] = compute_echograms_mics(room, src, rec, abs_wall, limits, mic_specs);

%% Rendering
% In this case all the information (receiver directivity especially) is
% already encoded in the echograms, hence they are rendered directly to
% discrete RIRs
fs = 48000;
mic_rirs = render_mic_rirs(abs_echograms, band_centerfreqs, fs);
...
mic_sigs = apply_source_signals_mics(mic_rirs, src_sigs);